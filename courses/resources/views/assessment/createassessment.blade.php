...
<div class="container mt-4 create-assessment-form">
    <h2>Create Peer Review Assessment for {{ $course->course_name }}</h2>

  

    <!-- Create Assessment Form -->
    <form method="POST" action="{{ route('assessment.store', $course->id) }}" class="mt-3">
        @csrf
        ...
        <div class="form-group mt-3">
            ...
            <textarea class="form-control" id="instruction" name="instruction" rows="3">{{ old('instruction') }}</textarea>
            @error('instruction')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <input type="number" class="form-control" id="number_of_reviews" name="number_of_reviews" value="{{ old('number_of_reviews') }}">
        ...
        <div class="form-group mt-3">
            ...
            <input type="number" class="form-control" id="max_score" name="max_score" value="{{ old('max_score') }}">
            @error('max_score')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
